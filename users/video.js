...
module.exports = async (api, event, regex) => {
	...
	api.sendMessage("Searching...", event.threadID, async (e, m) => {
		...
		try{
			...
			const yt_1 = /youtube.com\/watch\?v=([a-zA-Z0-9\-_]+)/
			...
			api.setMessageReactionMqtt("⏳", event.messageID, (e) => {}, true)
			api.editMessage("Processing...", msgID, (e, m) => {})
						...
						if(fs.existsSync(`${__dirname}/../temp/${event.threadID}_${event.senderID}.mp4`)){
							setTimeout(() => {
								fs.unlink(`${__dirname}/../temp/${event.threadID}_${event.senderID}.mp4`, (err) => {
									if(err){
										console.log(err)
										api.editMessage("Error", msgID, (e, m) => {})
									}
									api.setMessageReactionMqtt("", event.messageID, (e) => {}, true)
									console.log("Done")
									api.editMessage("File Uploaded", (e, m) => {})
								})
							}, 500)
						}
					})
